...
while not success and retry_count < 10:
    ...
    if 'http' in model.lower():
        ...
        run = subprocess.run(f'{SCRIPT_DIR}/download_hf_model.py --threads 2 --output "{output_folder}/{model_folder}" "{args.model}"', shell=True, check=False)
        write = model_folder
    else:
        ...
        break
    if run.returncode == 0:
        # Succesfull download. Write the model file or folder name to /tmp for use in --model arg
